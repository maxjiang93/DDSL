...
def simplex_ft_bw_gpu(dF, V, E, D, res, t, j, mode='density', gpuid=0):
    ...
    # invoke kernel function
    if n_dims == 2:
        simplex_ft_bw_kernel2[GDIM2, BDIM2](V_mem, E_mem, D_mem, C_mem, P_mem, omega_mem, dF_mem, dV_mem)
    elif n_dims == 3:
...
@cuda.jit
def simplex_ft_bw_kernel2(V, E, D, C, P, omega, dF, dV):
    r = dF.shape[:-1]
    j = 2
    n_elem = E.shape[0]
    n_dims = V.shape[1]
    ...
    for i in range(n_elem):
        for iu in range(start_u, int(r[0]), stride_u):
            ...
            for iv in range(start_v, int(r[1]), stride_v):
                ...
                uv[1] = u
                for ic in range(n_channel):
                    fac = simplex_ft_bw_device2(P[i], uv)
                    S = fac[-1]
                    for ip in range(j+1):
                        pnx = P[i, (ip+1)%(j+1), 0]
                        pny = P[i, (ip+1)%(j+1), 1]
                        pnnx = P[i, (ip+2)%(j+1), 0]
                        pnny = P[i, (ip+2)%(j+1), 1]
                        dV[E[i, ip], 0] += dF[iu, iv, ic] * (v*fac[ip]*C[i]-S*(pny-pnny))
                        dV[E[i, ip], 1] += dF[iu, iv, ic] * (u*fac[ip]*C[i]-S*(-pnx+pnnx))
                cuda.syncthreads()


@cuda.jit(device=True)
def simplex_ft_bw_device2(p, uvw):
    j = p.shape[0]-1
    n_dims = len(uvw)
    sig0 = 0
    sig1 = 0
