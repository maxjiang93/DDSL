...
class DDSL(Function):
    ...
    def forward(ctx, V, E, D, res, t, j, elem_batch=100, mode='density'):
        ...
        F_shape += [n_channel, 2]
        F = torch.zeros(*F_shape, dtype=torch.float32) # [dimX, dimY, dimZ, n_chan, 2] 2: real/imag
        ...
        for idx in range(ceil(n_vert/elem_batch)):
            ...
            for _ in range(n_dims):
                Di.unsqueeze_(dim=1) # [elem_batch, 1, 1, 1, n_channel, 1]
            tmp *= CDi # [elem_batch, dimX, dimY, dimZ, n_channel, 2]
            tmp = torch.sum(tmp, dim=0, keepdim=False) # [dimX, dimY, dimZ, n_channel, 2]
